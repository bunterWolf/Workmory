  ...
  private getMeetingTitleWindows(): string | null {
    try {
      ...
      const psScript = [
        ...
        '  public static List<string> Titles(int[] pids, out string fg) {',
        '    var r=new List<string>(); var ps=new HashSet<int>(pids);',
        '    IntPtr fgHwnd=GetForegroundWindow(); fg="";',
        '    EnumWindows((h,l)=>{',
        ...
        '      string title=(topmost?"TOPMOST:":"")+s.ToString();',
        '      r.Add(title);',
        '      if(h==fgHwnd)fg=s.ToString();',
        '      return true;',
        '    },IntPtr.Zero); return r;}',
        '}',
        '"@',
        '$pids=(Get-Process -Name \'*teams*\' -ErrorAction SilentlyContinue).Id',
        'if($pids){$fg="";$titles=[WE]::Titles($pids,[ref]$fg);Write-Output "FG:$fg";$titles}',
      ].join('\n');
